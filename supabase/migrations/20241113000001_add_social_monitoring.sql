...
-- 监控平台配置表
CREATE TABLE social_monitoring_platforms (
...
-- 创建索引
CREATE INDEX idx_monitoring_users_user_id ON monitoring_users(user_id);
CREATE INDEX idx_monitoring_users_platform ON monitoring_users(platform);
CREATE INDEX idx_monitoring_users_status ON monitoring_users(monitoring_status);
CREATE INDEX idx_monitoring_users_last_check ON monitoring_users(last_check_at);

CREATE INDEX idx_monitored_posts_monitoring_user_id ON monitored_posts(monitoring_user_id);
CREATE INDEX idx_monitored_posts_platform ON monitored_posts(platform);
CREATE INDEX idx_monitored_posts_published_at ON monitored_posts(published_at);
CREATE INDEX idx_monitored_posts_is_new ON monitored_posts(is_new);
CREATE INDEX idx_monitored_posts_notification_sent ON monitored_posts(notification_sent);

CREATE INDEX idx_webhook_configs_user_id ON webhook_configs(user_id);
CREATE INDEX idx_webhook_configs_type ON webhook_configs(webhook_type);

CREATE INDEX idx_monitoring_stats_user_id ON monitoring_stats(user_id);
CREATE INDEX idx_monitoring_stats_platform ON monitoring_stats(platform);
...
-- 行级安全策略（RLS）
ALTER TABLE social_monitoring_platforms ENABLE ROW LEVEL SECURITY;
ALTER TABLE monitoring_users ENABLE ROW LEVEL SECURITY;
...
-- 创建行级安全策略
CREATE POLICY "Users can view all platforms" ON social_monitoring_platforms
    FOR SELECT USING (true);

CREATE POLICY "Users can manage their monitoring users" ON monitoring_users
    FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Users can view their monitored posts" ON monitored_posts
    FOR SELECT USING (EXISTS (
        SELECT 1 FROM monitoring_users 
        WHERE monitoring_users.id = monitored_posts.monitoring_user_id 
        AND monitoring_users.user_id = auth.uid()
    ));

CREATE POLICY "Users can manage their webhook configs" ON webhook_configs
    FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Users can manage their monitoring rules" ON monitoring_rules
    FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Users can view their monitoring stats" ON monitoring_stats
    FOR ALL USING (auth.uid() = user_id);